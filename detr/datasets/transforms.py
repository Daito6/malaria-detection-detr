...
class RandomGaussianBlur:
    def __init__(self, kernel_size=(5, 9), sigma=(0.1, 2.0), p=0.5):
        self.kernel_size = random.choice(range(kernel_size[0], kernel_size[1]+1, 2))
        self.sigma = sigma
        self.p = p
...
class RandomBrightnessContrast:
    ...
    def __call__(self, img, target):
        if random.random() < self.p:
            factor_b = random.uniform(1 - self.brightness, 1 + self.brightness)
            factor_c = random.uniform(1 - self.contrast, 1 + self.contrast)
            img = F.adjust_brightness(img, factor_b)
            img = F.adjust_contrast(img, factor_c)
        return img, target
